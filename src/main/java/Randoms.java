...
public class Randoms {
    ...
    public double ran1(long[] idum){
        ...
        if (idum[0] <= 0 || iy == 0) {           // Initialize.
            if (-(idum[0]) < 1) idum[0]=1;     // Be sure to prevent idum = 0.
		    else idum[0] = -(idum[0]);
	    ...
	    idum[0]=IA*(idum[0]-k*IQ)-IR*k;       // Compute idum=(IA*idum) % IM without over-
        if (idum[0] < 0) idum[0] += IM;       // flows by Schrage's method.
    ...
    public double gaussdev(long[] idum) {
        ...
        if (idum[0] < 0) iset=0;      //     Reinitialize.
